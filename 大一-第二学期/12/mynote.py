...
class SimpleEdit:
    def __init__(self,master):
        ...
        self.editm.add_separator()
        self.editm.add_command(label='查找',command=self.find)
        self.editm.add_command(label='替换',command=self.replace)
        self.editm.add_separator()
    ...
    def replace(self):
        self.st.tag_config('match', foreground='red', background='yellow')
        str = simpledialog.askstring(title='替换', prompt='请输入你要查找的字符串和要替换的字符串，用分号分开：')
        temp=str.split(';')
        findstr=temp[0]
        replacestr=temp[1]
        start_pos = '1.0'
    ...
    def delete(self):
        try:
            ...
            messagebox.showerror(title='错误提示',message='你没有选择要删除的内容 ！')
    def select_all(self):
        self.st.event_generate('<<SelectAll>>')
    def close(self):
        self.window.destroy()
    def about(self):
        messagebox.showinfo(title='版本信息',message='版本1.0；版权所有：学号')
    def saveasfile(self):
